...
vi.mock('@wip/shared', () => ({
	...
	createBranchForChild: vi.fn(async (_dir: string, child: {branch?: string; subject: string}, _project: string) => child.branch ?? child.subject.toLowerCase().replaceAll(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')),
