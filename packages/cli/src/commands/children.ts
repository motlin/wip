...
export default class Children extends Command {
  ...
  async run(): Promise<ChildrenJson> {
    ...
    const passed = results.reduce(
      (sum, r) => sum + r.children.filter((c) => c.testStatus === "passed").length,
      0,
    );
    const failed = results.reduce(
      (sum, r) => sum + r.children.filter((c) => c.testStatus === "failed").length,
      0,
    );
    const unknown = results.reduce(
      (sum, r) => sum + r.children.filter((c) => c.testStatus === "unknown").length,
      0,
    );
