...
export default class Push extends Command {
  ...
  static override description =
    "Push green children: create branches, push, and optionally create PRs";
      ...
      for (const child of green) {
        ...
        if (!child.branch) {
          ...
          log.subprocess.debug(
            {
              cmd: "git",
              args: ["-C", p.dir, "branch", branchName, child.sha],
              duration: branchDuration,
            },
            `git -C ${p.dir} branch ${branchName} ${child.sha} (${branchDuration}ms)`,
          );
        }
        ...
        const pushResult = await execa(
          "git",
          ["-C", p.dir, "push", "-u", p.upstreamRemote, `${child.sha}:refs/heads/${branchName}`],
          {
            reject: false,
          },
        );
        ...
        log.subprocess.debug(
          {
            ...
            args: [
              ...
              "-u",
              p.upstreamRemote,
              `${child.sha}:refs/heads/${branchName}`,
            ],
            duration: pushDuration,
          },
          `git -C ${p.dir} push -u ${p.upstreamRemote} ${child.sha}:refs/heads/${branchName} (${pushDuration}ms)`,
        );
  ...
  private async createPR(
    dir: string,
    branch: string,
    child: { shortSha: string; subject: string },
    base: string,
  ): Promise<string | undefined> {
    const existingPR = await execa(
      "gh",
      ["pr", "view", branch, "--repo", ".", "--json", "number"],
      {
        cwd: dir,
        reject: false,
      },
