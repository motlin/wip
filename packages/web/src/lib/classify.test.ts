...
describe("classifyCommit", () => {
  ...
  it("returns ready_to_push when test passed", () => {
    expect(classifyCommit(makeCommit({ testStatus: "passed" }), makeProject())).toBe(
      "ready_to_push",
    );
  });
  ...
  it("returns test_running when test is running", () => {
    expect(classifyCommit(makeCommit({ testStatus: "running" }), makeProject())).toBe(
      "test_running",
    );
  });
  ...
  it("prioritizes test_running over detached_head", () => {
    expect(
      classifyCommit(makeCommit({ testStatus: "running" }), makeProject({ detachedHead: true })),
    ).toBe("test_running");
  });

  it("prioritizes test_failed over detached_head", () => {
    expect(
      classifyCommit(makeCommit({ testStatus: "failed" }), makeProject({ detachedHead: true })),
    ).toBe("test_failed");
  });
});
...
describe("classifyBranch", () => {
  ...
  it("returns needs_split for multi-commit branch with tests passed", () => {
    expect(
      classifyBranch(makeBranch({ testStatus: "passed", commitsAhead: 3 }), makeProject()),
    ).toBe("needs_split");
  });

  it("returns ready_to_push when commitsAhead is undefined (defaults to single)", () => {
    expect(classifyBranch(makeBranch({ testStatus: "passed" }), makeProject())).toBe(
      "ready_to_push",
    );
  });
  ...
  it("returns ready_to_push for commitsAhead=0", () => {
    expect(
      classifyBranch(makeBranch({ testStatus: "passed", commitsAhead: 0 }), makeProject()),
    ).toBe("ready_to_push");
  });

  it("returns needs_split for commitsAhead=2", () => {
    expect(
      classifyBranch(makeBranch({ testStatus: "passed", commitsAhead: 2 }), makeProject()),
    ).toBe("needs_split");
  });

  it("returns pushed_no_pr when pushed and in sync with remote", () => {
    expect(
      classifyBranch(makeBranch({ pushedToRemote: true, localAhead: false }), makeProject()),
    ).toBe("pushed_no_pr");
  });

  it("returns ready_to_push when pushed but local is ahead of remote", () => {
    expect(
      classifyBranch(makeBranch({ pushedToRemote: true, localAhead: true }), makeProject()),
    ).toBe("ready_to_push");
  });

  it("returns pushed_no_pr when pushed and localAhead is undefined (defaults to in-sync)", () => {
    expect(classifyBranch(makeBranch({ pushedToRemote: true }), makeProject())).toBe(
      "pushed_no_pr",
    );
  });
  ...
  it("prioritizes test_failed over needs_rebase", () => {
    expect(
      classifyBranch(makeBranch({ testStatus: "failed", needsRebase: true }), makeProject()),
    ).toBe("test_failed");
  });

  it("prioritizes skippable over test_failed", () => {
    expect(
      classifyBranch(makeBranch({ skippable: true, testStatus: "failed" }), makeProject()),
    ).toBe("skippable");
  });

  it("returns rebase_conflicts when needsRebase and not rebaseable", () => {
    expect(
      classifyBranch(makeBranch({ needsRebase: true, rebaseable: false }), makeProject()),
    ).toBe("rebase_conflicts");
  });

  it("returns needs_rebase when needsRebase and rebaseable", () => {
    expect(classifyBranch(makeBranch({ needsRebase: true, rebaseable: true }), makeProject())).toBe(
      "needs_rebase",
    );
  });
  ...
  it("returns test_failed when test failed even with dirty project", () => {
    expect(classifyBranch(makeBranch({ testStatus: "failed" }), makeProject({ dirty: true }))).toBe(
      "test_failed",
    );
  });

  it("returns test_running when test is running", () => {
    expect(classifyBranch(makeBranch({ testStatus: "running" }), makeProject())).toBe(
      "test_running",
    );
  });
  ...
  it("returns pushed_no_pr when pushed, in sync, and test passed", () => {
    expect(
      classifyBranch(
        makeBranch({ pushedToRemote: true, localAhead: false, testStatus: "passed" }),
        makeProject(),
      ),
    ).toBe("pushed_no_pr");
  });
...
describe("classifyPullRequest", () => {
  ...
  it("returns checks_failed when checks failed, even if approved", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "failed", reviewStatus: "approved" }))).toBe(
      "checks_failed",
    );
  });
  ...
  it("returns checks_failed when checks failed, even with changes_requested", () => {
    expect(
      classifyPullRequest(makePR({ checkStatus: "failed", reviewStatus: "changes_requested" })),
    ).toBe("checks_failed");
  });

  it("returns checks_running when checks running, even if approved", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "running", reviewStatus: "approved" }))).toBe(
      "checks_running",
    );
  });

  it("returns checks_running when checks pending, even if approved", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "pending", reviewStatus: "approved" }))).toBe(
      "checks_running",
    );
  });

  it("returns approved only when checks passed AND review approved", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "passed", reviewStatus: "approved" }))).toBe(
      "approved",
    );
  });
  ...
  it("returns changes_requested when checks passed and changes requested", () => {
    expect(
      classifyPullRequest(makePR({ checkStatus: "passed", reviewStatus: "changes_requested" })),
    ).toBe("changes_requested");
  });

  it("returns review_comments when checks passed and commented", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "passed", reviewStatus: "commented" }))).toBe(
      "review_comments",
    );
  });

  it("returns checks_passed when checks passed and no review", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "passed", reviewStatus: "no_pr" }))).toBe(
      "checks_passed",
    );
  });

  it("returns checks_running when checks running with no review", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "running", reviewStatus: "no_pr" }))).toBe(
      "checks_running",
    );
  });

  it("returns checks_unknown for unknown check status", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "unknown", reviewStatus: "no_pr" }))).toBe(
      "checks_unknown",
    );
  });

  it("returns checks_unknown for none check status", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "none", reviewStatus: "no_pr" }))).toBe(
      "checks_unknown",
    );
  });

  it("returns checks_unknown when approved but checks unknown", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "unknown", reviewStatus: "approved" }))).toBe(
      "checks_unknown",
    );
  });

  it("returns ready_to_push when checks failed and local is ahead of remote", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "failed", localAhead: true }))).toBe(
      "ready_to_push",
    );
  });

  it("returns checks_failed when checks failed and in sync with remote", () => {
    expect(classifyPullRequest(makePR({ checkStatus: "failed", localAhead: false }))).toBe(
      "checks_failed",
    );
  });
  ...
  it("returns needs_rebase when PR needs rebase", () => {
    expect(classifyPullRequest(makePR({ needsRebase: true, checkStatus: "passed" }))).toBe(
      "needs_rebase",
    );
  });
  ...
  it("returns rebase_conflicts when PR needs rebase and is not rebaseable", () => {
    expect(
      classifyPullRequest(makePR({ needsRebase: true, rebaseable: false, checkStatus: "passed" })),
    ).toBe("rebase_conflicts");
  });
  ...
  it("prioritizes needsRebase over check/review status", () => {
    expect(
      classifyPullRequest(
        makePR({ needsRebase: true, checkStatus: "passed", reviewStatus: "approved" }),
      ),
    ).toBe("needs_rebase");
  });
});
...
describe("State machine consistency", () => {
  ...
  it("every non-special category in schema appears in STATE_MACHINE as from or to state", () => {
    const missingStates = validCategories.filter(
      (cat) => !statesInMachine.has(cat) && !specialStates.has(cat),
    );
  ...
  it("every state in STATE_MACHINE is a valid category", () => {
    const invalidStates = Array.from(statesInMachine).filter(
      (state) => !validCategories.includes(state as any),
    );
