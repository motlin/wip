...
function collectFiles(dir: string, out: string[] = []): string[] {
	for (const entry of fs.readdirSync(dir, {withFileTypes: true})) {
		...
		} else if (/\.tsx?$/.test(entry.name) && !/\.test\./.test(entry.name) && !/\.d\./.test(entry.name)) {
			out.push(full);
		}
	}
	return out;
...
describe("client bundle has no node: imports", () => {
	it("files with named node: imports are only dynamically imported", () => {
		...
		const fileContents = new Map(allFiles.map((file) => [file, fs.readFileSync(file, "utf-8")] as const));
