...
async function runTest(job: TestJob): Promise<void> {
  ...
  log.subprocess.debug(
    { cmd: "git", args: ["-C", job.projectDir, "test", "run", "--retest", job.sha], duration },
    `git -C ${job.projectDir} test run --retest ${job.sha} (${duration}ms)`,
  );
  ...
  job.message =
    status === "passed"
      ? `${job.shortSha} passed`
      : `${job.shortSha} failed (exit ${result.exitCode})`;
