...
import {projectsQueryOptions, snoozedQueryOptions} from '../../lib/queries';
import {useWorkItemCounts} from '../../lib/use-work-items';
...
function Home() {
	...
	const counts = useWorkItemCounts(projects);
		...
		<div className="mx-auto max-w-2xl p-6">
			<div className="mb-8">
				...
				<p className="mt-1 text-sm text-text-500">
					{counts.total} items across {counts.projectCount} projects
				</p>
			</div>
			<div className="grid grid-cols-2 gap-4">
				...
				>
					...
					<div className="mt-3 flex gap-3 text-xs">
						{counts.pullRequests > 0 && (
							<span className="text-blue-600 dark:text-blue-400">{counts.pullRequests} PRs</span>
						)}
						{counts.branches > 0 && (
							<span className="text-green-600 dark:text-green-400">{counts.branches} branches</span>
						)}
						{counts.todos > 0 && (
							<span className="text-text-500">{counts.todos} todos</span>
						)}
					</div>
				</Link>
				<Link
				...
				>
					...
					<div className="mt-3 flex gap-3 text-xs">
						<span className="text-text-500">{counts.total} cards</span>
					</div>
				</Link>
				{snoozed.length > 0 && (
					<Link
