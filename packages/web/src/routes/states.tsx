...
import {STATE_MACHINE, type Category, type StateTransition} from '@wip/shared';
					...
					{CATEGORY_PRIORITY.map((cat, idx) => {
						...
						return (
							...
							>
								...
								<td className={`px-3 py-2 font-medium ${config.color}`}>
									{config.label}
								</td>
								<td className="px-3 py-2">
		...
		<div className="mx-auto max-w-7xl p-6">
			<div className="mb-6">
				...
				<p className="mt-1 text-sm text-text-400">
					...
					{activeState && (
						<>
							{' '}Highlighting: <code className="rounded bg-amber-500/20 px-1 py-0.5 font-mono text-xs text-amber-400">{activeState}</code>
							<Link to="/states" className="ml-2 text-xs text-text-500 hover:text-text-200">(clear)</Link>
						</>
					)}
				</p>
			</div>
